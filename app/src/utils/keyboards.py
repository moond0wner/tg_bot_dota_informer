...
async def paginated_buttons(page: int,
                            number_of_page: int,
    ...
    try:
        ...
        if previous_page:
            navigation_buttons.append(InlineKeyboardButton(text='<<', callback_data=f"page:{page - 1}"))
        ...
        keyboard.row(InlineKeyboardButton(text=f'{page + 1}/{number_of_page}', callback_data='dummy_action'))
...
async def account_buttons(page: int,
    ...
    try:
        ...
        for account_id, name in accounts_on_page.items():
            keyboard.row(InlineKeyboardButton(text=name, callback_data=f'account_id:{account_id}'))
        ...
        if previous_page:
            navigation_buttons.append(InlineKeyboardButton(text='<<', callback_data=f'carousel:{page - 1}'))

        if next_page:
            navigation_buttons.append(InlineKeyboardButton(text=">>", callback_data=f"carousel:{page + 1}"))

        keyboard.row(InlineKeyboardButton(text=f'{page + 1}/{number_of_pages}', callback_data='dummy_action'))
